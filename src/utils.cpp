...
QImage idToColor(const QImage &image_id, const Id2Labels& id_label) {
    ...
    // Bycycle to avoid 1px black frame
    result = result.copy(QRect(1, 1, image_id.size().width() - 2, image_id.size().height() - 2));
    // by default nearest interpolation used in Qt scaled
    result = result.scaled(image_id.size());
...
void idToColor(const QImage &image_id, const Id2Labels& id_label, QImage *result) {
	...
	uchar * pix;
    for (ushort y = 0; y < image_id.height(); ++y) {
        const uchar* line_in = image_id.scanLine(y);
        uchar* line_out = result->scanLine(y);
        for (ushort x = 0; x < image_id.width() * 3; x += 3) {

            const ushort id = line_in[x];
			QMap<int, const LabelInfo*>::const_iterator it = id_label.find(id);

			if (it != id_label.end()) {
				pix = &line_out[x];
                const LabelInfo* label = *it;
                *pix = static_cast<uchar>(label->color.red());
                ++pix;
                *pix = static_cast<uchar>(label->color.green());
                ++pix;
